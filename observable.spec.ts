...
import { debounce, DebouncedObservable, Observable } from './index';
...
describe('Observable', () => {
  ...
  it('notifies immediately if option set', () => {
    let notifiedValue = 0;
    const observable = new Observable(1);
    observable.subscribe((value) => (notifiedValue = value), { immediate: true });
    assert.equal(notifiedValue, 1);
    assert.equal(notifiedValue, observable.value);
  });
